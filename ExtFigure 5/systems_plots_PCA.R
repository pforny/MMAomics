...
# g_prot_pca_dens <-
# lapply(pcs,function(xvar){
# 	ggplot(df_out_prot, aes(x = eval(parse(text=xvar)), color = type, fill = type)) +
# 		geom_density(alpha = 0.5) +
# 		theme_bw()
# })
...
# pca_prot <- ggarrange(g_prot_pca, g_prot_pca_dens, scree_plot, ncol = 1)

# ggsave("Figs/systems_plots/pca_plots_prot_scalerRound.png", pca_prot, device = png(), width = 140, height = 220, units = c("mm"), dpi = 600)
# dev.off()
...
# pca_rna <- ggarrange(g_rna_pca, g_rna_pca_dens, scree_plot_rna, ncol = 1)

# ggsave("Figs/systems_plots/pca_plots_rna_colnormed.png", pca_rna, device = png(), width = 120, height = 220, units = c("mm"), dpi = 600)
# dev.off()
